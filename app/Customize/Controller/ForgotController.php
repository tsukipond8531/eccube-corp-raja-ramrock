...
class ForgotController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $tipQuery = $form['password_tip_query']->getData();
            $tipAnswer = $form['password_tip_answer']->getData();

            if (!is_null($Customer)) {
                if ( $tipQuery != $Customer->getPasswordTipQuery() || $tipAnswer != $Customer->getPasswordTipAnswer() ) {
                    return [
                        'form' => $form->createView(),
                        'validation_error' => 'tip',
                    ];
                }
                // リセットキーの発行・有効期限の設定
                $Customer
